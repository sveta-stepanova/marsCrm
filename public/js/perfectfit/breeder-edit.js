...
(function () {
    ...
    _jquery2.default.mask.definitions = {
        '8': '[0-9]',
        'a': '[A-Za-z]',
        '*': '[A-Za-z0-9]'
    };
    ...
    (0, _jquery2.default)(function () {
        ...
        (0, _jquery2.default)('.edit-ld').on('click', function () {
            var form = (0, _jquery2.default)(this).closest('.change_data');
        ...
        (0, _jquery2.default)('.change_data').on('click', '.remove-ld', function () {
            var form = (0, _jquery2.default)(this).closest('.change_data');
        ...
        // {{{ Masks
        (0, _jquery2.default)('input[data-mask]').each(function (i, input) {
        ...
        (0, _jquery2.default)('select[data-chosen]').each(function (_, el) {
            var $el = (0, _jquery2.default)(el),
                options = $el.attr('id').match(/^Breed/) ? {} : { width: 300 };
            $el.chosen(options);
        });
        var edit = (0, _jquery2.default)('#header').data('edit');

        // {{{ Checkboxes. Some plugin did this in old version, but I couldn't find it in NPM :(
        (0, _jquery2.default)('span.checkbox').click(function (e) {
            var $span = (0, _jquery2.default)(e.target),
        ...
        var uploading = false;
        var $form = (0, _jquery2.default)('form#profile'),
            $FamilyTree = (0, _jquery2.default)('#FamilyTree'),
            $FamilyTreeFileId = (0, _jquery2.default)('#FamilyTreeFileId'),
            token = $form.find('input[name=_token]').val(),
            $submitBtn = (0, _jquery2.default)('#Submit');
        ...
        $submitBtn.click(function () {
            ...
            _jquery2.default.post({
                ...
                for (var field in err.errors) {
                    var _ref = field.match(/^Breed\.([0-9]+)\./) || [null, null],
                        _ref2 = _slicedToArray(_ref, 2),
                        _ = _ref2[0],
                        breedNum = _ref2[1],
                        errStr = err.errors[field].join('<br/>');
                    if (field === 'Breed') {
                        (0, _jquery2.default)('#BreedMessage').html(errStr);
                    } else if (breedNum) {
                        $breeds.find('.breed_table').eq(breedNum).find('.message').html(errStr);
                    } else if (field == 'TermsAccepted' || field == 'RulesAccepted' || field == 'Agree18') {
                        (0, _jquery2.default)('#' + field).parent().parent().find('.message').html(errStr);
                    } else {
                        (0, _jquery2.default)('#' + field).parent().find('.message').html(errStr);
                    }
                }
                // }}}
            });
        ...
        // {{{ Geography
        var $CitySelect = (0, _jquery2.default)('#NurseryCityId'),
            $RegionSelect = (0, _jquery2.default)('#NurseryRegionId');

        var updateCitiesList = function updateCitiesList(regionId) {
            $CitySelect.prop('disabled', true).empty().trigger('chosen:updated');
            _jquery2.default.ajax('/register/cities/' + regionId, { success: function success(_ref3) {
                    var cities = _ref3.cities;

                    (0, _jquery2.default)('<option></option>').appendTo($CitySelect);
                    cities.forEach(function (city) {
                        var $option = (0, _jquery2.default)('<option></option>').attr('value', city.Id).html(city.FlatShortName);
                        $CitySelect.append($option);
                    });
                    $CitySelect.prop('disabled', false).attr('data-placeholder', '-- выберите город --').trigger('chosen:updated');
                } });
        };
        ...
        // {{{ Breeds
        var $breeds = (0, _jquery2.default)('#Breeds'),
            $breedSelects = $breeds.find('select');
        var breeds = [],
            breedsById = {},
            selectedBreedIds = [];
        var fillBreeds = function fillBreeds(except) {
            $breedSelects.each(function (i, el) {
                if (i === except) {
                    return;
                }
                var $select = (0, _jquery2.default)(el),
                    value = $select.val();
                $select.empty().append((0, _jquery2.default)('<option></option>'));
                breeds.forEach(function (breed) {
                    if (selectedBreedIds.includes(breed.Id) && selectedBreedIds.indexOf(breed.Id) !== i) {
                        return;
                    }
                    $select.append((0, _jquery2.default)('<option></option>').html(breed.Name).attr('value', breed.Id));
                });
                $select.val(value).trigger('chosen:updated');
            });
        };
        _jquery2.default.ajax('/register/breeds', { success: function success(res) {
                breeds = res.breeds;
                breeds.forEach(function (breed) {
                    return breedsById[breed.Id] = breed;
                });
                fillBreeds();
                if ($breeds.data('breeds')) {
                    var existingBreeds = $breeds.data('breeds');
                    existingBreeds.forEach(function (existingBreed, i) {
                        var $formRow = $breeds.find('.breed_table').slice(i, i + 1),
        ...
        $breedSelects.on('input', function (e) {
            var $this = (0, _jquery2.default)(e.target),
                breed = breedsById[$this.val()] || null,
                $breedDetails = $this.parent().parent().find('input'),
                $quantity = $this.parent().parent().find('input').not('.chosen-search-input').eq(0),
                $weight = $this.parent().parent().find('input').not('.chosen-search-input').eq(1);
            selectedBreedIds = [];
            $breedSelects.each(function (i, select) {
                return selectedBreedIds.push((0, _jquery2.default)(select).val());
            });
...
//                $breedDetails.attr('disabled', 'disabled');
            ...
            var currentWeight = parseFloat($weight.val()),
        ...
        $breeds.find('input[id*=Quantity], input[id*=Weight]').change(function (e) {
            var $this = (0, _jquery2.default)(e.target),
            ...
            var monthlyConsumption = Math.pow(weight, 0.75) * 95 * quantity * 0.7 / 10 / 12;
            $msgOutput.html("70% \u043E\u0442 \u043E\u0431\u0449\u0435\u0439 \u043F\u043E\u0442\u0440\u0435\u0431\u043D\u043E\u0441\u0442\u0438 \u0432\u0430\u0448\u0435\u0433\u043E \u043F\u043E\u0433\u043E\u043B\u043E\u0432\u044C\u044F \u0441\u043E\u0441\u0442\u0430\u0432\u043B\u044F\u044E\u0442 " + Math.round(10 * monthlyConsumption) / 10 + " \u043A\u0433. \u043A\u043E\u0440\u043C\u0430 \u0432 \u043C\u0435\u0441\u044F\u0446");
        });
        // }}}
    });
}).call(window);
